...
class LiamChenAgent(Agent):
    ...
    async def on_enter(self):
        ...
        if self.call_direction == "outbound":
            self.session.say(
                text=clean_for_tts(f"Hi {self.lead_name}, this is Liam from {hotel_name}. I'm following up on your recent stay — do you have a moment?")
            )
        else:
            self.session.say(
                text=clean_for_tts(f"Thank you for calling {hotel_name}, this is Liam from customer success. I'm here to help — what can I do for you?")
            )
