...
class MarcusReedAgent(Agent):
    ...
    async def on_enter(self):
        ...
        if self.call_direction == "outbound":
            self.session.say(
                text=clean_for_tts(f"Hey {self.lead_name}! Marcus Reed here from {hotel_name}. Got 30 seconds? I think I've got something you'll love.")
            )
        else:
            self.session.say(
                text=clean_for_tts(f"Thanks for calling {hotel_name}, Marcus Reed here! What can I do for you?")
            )
