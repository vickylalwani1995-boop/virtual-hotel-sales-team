...
class DonnaMarieAgent(Agent):
    ...
    async def on_enter(self):
        ...
        if self.call_direction == "outbound":
            self.session.say(
                text=clean_for_tts(f"Hi {self.lead_name}, this is Donna Marie, Director of Sales at {hotel_name}. Do you have 30 seconds?")
            )
        else:
            self.session.say(
                text=clean_for_tts(f"Thank you for calling {hotel_name}, this is Donna Marie. How can I help you today?")
            )
