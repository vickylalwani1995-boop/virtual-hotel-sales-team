  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-8">
      ...
      >
        ...
        <span
          className={`absolute top-4 left-6 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase ${
            agent.tier === 1
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-6">
        ...
        <aside className="space-y-4">
          <div className="bg-white rounded-2xl border border-mhsp-line p-6 shadow-[0_2px_10px_-4px_rgba(11,36,71,0.06)]">
        ...
        <div className="space-y-4">
          {/* Action card */}
          <div className="bg-white rounded-2xl border border-mhsp-line p-5 shadow-[0_2px_10px_-4px_rgba(11,36,71,0.06)]">
            {!profile ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button
                  onClick={runAgent}
                  disabled={running}
                  className="inline-flex items-center gap-2 rounded-lg bg-mhsp-gold hover:bg-mhsp-gold-soft disabled:opacity-40 disabled:cursor-not-allowed px-5 py-2.5 text-sm font-semibold text-mhsp-navy shadow-[0_4px_14px_-4px_rgba(212,165,55,0.5)] transition-all"
                >
                  {running ? (
                    <>
          ...
          {output && (
            <motion.div
              ...
              className="space-y-3"
            >
              <div className="bg-white rounded-2xl border border-mhsp-line p-4 flex flex-wrap items-center gap-2 shadow-[0_2px_10px_-4px_rgba(11,36,71,0.06)]">
                <DownloadButtons
                  output={output}
                  basename={agent.id}
                  showExcel={isLeadGen}
                ...
                {outputIsSample && (
                  <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-mhsp-cream-warm border border-mhsp-line/60 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-mhsp-muted">
                    Sample
                  </span>
                )}
              </div>
...
function ProfileChips({ profile }: { profile: string }) {
  ...
  if (rows.length === 0) {
    return (
      <div className="mt-5 pt-5 border-t border-mhsp-line/60">
        <p className="text-[10px] font-semibold tracking-[0.18em] text-mhsp-gold uppercase mb-2">
          Working with
        </p>
        <p className="text-xs text-mhsp-muted leading-relaxed line-clamp-6">
          {profile}
        </p>
      </div>
    );
  ...
  return (
    <div className="mt-6 pt-6 border-t border-mhsp-line/60">
      <p className="text-[10px] font-semibold tracking-[0.18em] text-mhsp-gold uppercase mb-3">
        Working with
      </p>
      <dl className="space-y-2.5">
        {rows.map((r) => (
          <div key={r.label} className="flex flex-col gap-0.5">
            <dt className="text-[10px] font-semibold uppercase tracking-wider text-mhsp-muted">
              {r.label}
            </dt>
...
function EmptyState({ agentName, icon }: { agentName: string; icon: string }) {
  return (
    <div className="bg-mhsp-cream-warm/40 border border-mhsp-line border-dashed rounded-2xl p-10 text-center">
      ...
      <p className="text-xs text-mhsp-muted mt-2">— {agentName}</p>
    </div>
  );
}
...
function SkeletonLoader() {
  return (
    <div className="bg-white rounded-2xl border border-mhsp-line p-6 sm:p-8 shadow-[0_2px_10px_-4px_rgba(11,36,71,0.06)]">
